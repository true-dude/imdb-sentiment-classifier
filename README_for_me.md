...
При использовании токенайзера (`imdb_sentiment_classifier/bpe_tokenizer.py`):
...
TextCNN — это свёрточная нейросеть, только не для картинок, а для текста (`imdb_sentiment_classifier/cnn.py`).
...
### 4.2. Обучение из консоли через CLI `train-cnn`

CLI-команда `train-cnn` (в пакете `imdb_sentiment_classifier.scripts.train_cnn`) делает следующее:
...
Запуск с настройками по умолчанию (из корня репозитория, чтобы `configs/` был виден):
...
uv run train-cnn train.epochs=5 train.lr=5e-4 data.max_length=256

# изменить число BPE-слияний
uv run train-cnn tokenizer.num_merges=2000

# логирование в MLflow
uv run train-cnn mlflow.enabled=true mlflow.mlflow_uri=http://localhost:5000
```

Примечания:

- Если `artifacts/tokenizer.json` уже существует, `train-cnn` загрузит его и не будет заново обучать BPE (экономит время). Если его нет, токенайзер обучится и будет сохранён.
...
### 5.1. Инференс через CLI `infer-cnn`
...
Запускай из корня репозитория (конфиги Hydra лежат в `./configs` и не упакованы в пакет).
...
- перед этим должен быть запущен `train-cnn`, чтобы существовали:
...
# изменить max_length
uv run infer-cnn data.max_length=256 text="Absolutely fantastic, would watch again!"
```
...
- По умолчанию DVC remote `myremote` указывает на локальный путь `../remote` (см. `.dvc/config`). Он лежит рядом с репозиторием, без авторизации.
- Конфиг для MinIO сохранён как `remote "minio"` (не дефолтный); если нужно использовать MinIO из `docker-compose`, переключите: `dvc remote default minio` и поправьте `endpointurl` на `http://minio:9000` внутри контейнера.
- Чтобы загрузить данные в дефолтный локальный remote:
  ```bash
  dvc add data/IMDb
  dvc push data/IMDb.dvc
  ```
- `download_data()` в CLI `train-cnn`/`infer-cnn` пытается стянуть `data/IMDb` через `dvc pull` (использует remote из `.dvc/config` или из переменной `DVC_REMOTE`). Если данных нет в DVC, но есть Kaggle CSV, соберёт локально.
...
- Обучение в контейнере:
  ...
  train-cnn mlflow.enabled=true mlflow.mlflow_uri=http://mlflow:8080
  ```
...
train-cnn mlflow.enabled=true mlflow.mlflow_uri=http://mlflow:5000

# инференс (использует сохранённые артефакты в текущей директории)
infer-cnn text="Great movie!" paths.tokenizer_path=artifacts/tokenizer.json
```

Важно: параметр `mlflow.mlflow_uri` должен указывать на сервис `mlflow` из compose (`http://mlflow:5000`), а артефакты/чекпоинты сохраняются в общую volume (`.` смонтирована в `/app`).
...
```bash
uv run infer-cnn 'text="Great movie!"'
# или с явными путями
uv run infer-cnn \
  paths.checkpoint_path=checkpoints/textcnn_imdb.pt \
  paths.tokenizer_path=artifacts/tokenizer.json \
...
```bash
uv run export-onnx paths.checkpoint_path=checkpoints/textcnn_imdb.pt paths.tokenizer_path=artifacts/tokenizer.json paths.onnx_path=artifacts/textcnn.onnx data.max_length=128
```
...
```bash
uv run export-tensorrt paths.onnx_path=artifacts/textcnn.onnx paths.trt_path=artifacts/textcnn.trt data.max_length=128   # при желании переопределить max_length
```
...
from imdb_sentiment_classifier.bpe_tokenizer import BPETokenizer
...
Есть CLI `triton-client` (скрипт `imdb_sentiment_classifier/scripts/triton_client.py`), который сам токенизирует текст и шлёт запрос в Triton.

```bash
uv run triton-client --text "Great movie!" --tokenizer-path artifacts/tokenizer.json --url http://localhost:8000/v2/models/textcnn/infer --max-length 128
```
